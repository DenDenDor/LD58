    ...
    class LayoutsMod
    {
        ...
        internal static void InitProperties()
        {
            if (WindowLayout == null)
            {
                try 
                {
                    WindowLayout = typeof(EditorWindow).Assembly.GetType("UnityEditor.WindowLayout");
                    if (WindowLayout == null) 
                    {
                        Debug.LogError("Failed to find WindowLayout type");
                        return;
                    }

                    LoadWindowLayout = WindowLayout.GetMethod("LoadWindowLayout", 
                        new[] { typeof(string), typeof(bool), typeof(bool), typeof(bool) });
                    SaveWindowLayout = WindowLayout.GetMethod("SaveWindowLayout", ~(BindingFlags.Instance));
                    layoutsModePreferencesPath = WindowLayout.GetProperty("layoutsModePreferencesPath", ~(BindingFlags.Instance));

                    if (LoadWindowLayout == null || SaveWindowLayout == null || layoutsModePreferencesPath == null)
                    {
                        Debug.LogError("Failed to find required WindowLayout methods/properties");
                    }
                }
                catch (Exception ex)
                {
//                    Debug.LogError($"Error initializing LayoutsMod properties: {ex}");
        ...
        string[] get_defaults {
            get {
                ...
                // Add null check for layoutsModePreferencesPath
                var layoutsPath = layoutsModePreferencesPath?.GetValue(null, null) as string;
                if (string.IsNullOrEmpty(layoutsPath) || !Directory.Exists(layoutsPath)) 
                {
                    return (_get_defaults = new string[0]);
                }

                try 
                {
                    _get_defaults = Directory.GetFiles(layoutsPath, "*.wlt")
                        .Where(path => path.EndsWith("Default.wlt", StringComparison.OrdinalIgnoreCase) ||
                ...
                {
                    _get_defaults = new string[0];
                }

                // Ensure we always return an array of at least 30 elements
        ...
        string[] GET_SAVED()
        {
            ...
            var defaults = get_defaults ?? new string[30]; // Ensure defaults is never null
            var r = new List<string>();
    
            for (int i = 0; i < 10; i++)
            {
                var l = Root.p[0].par_e.GET("INTERNAL_LAYOUT_" + i.ToString(), i < defaults.Length ? defaults[i] : "");
                if (string.IsNullOrEmpty(l)) continue;
        
                int tl = -1;
                if ((tl = l.LastIndexOf('?')) != -1 && tl != l.Length - 1) 
                {
                    l = d + l.Substring(tl + 1);
                }
                r.Add(l);
            }
